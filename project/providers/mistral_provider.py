...
DEFAULT_BUZZ_MISTRAL_LARGE_API = "https://mistral-large-3-675b-instruct-nvfp4-3e1ds.inference.buzzperformancecloud.com"


class MistralProvider(BaseLLMProvider):
    """
    Provider for Mistral chat-completions API.

    Defaults to https://mistral-large-3-675b-instruct-nvfp4-3e1ds.inference.buzzperformancecloud.com; override with BUZZ_MISTRAL_LARGE_API or base_url.
    API key is read from BUZZ_MISTRAL_LARGE_AUTH_TOKEN or api_key argument.
    """
    ...
    def __init__(
        ...
        **kwargs
        ...
        base_url = (base_url or os.getenv("BUZZ_MISTRAL_LARGE_API") or DEFAULT_BUZZ_MISTRAL_LARGE_API).strip().rstrip("/")
        api_key = api_key or os.getenv("BUZZ_MISTRAL_LARGE_AUTH_TOKEN")
        if not api_key:
            raise ValueError(
                "Mistral api_key must be set via the api_key argument or BUZZ_MISTRAL_LARGE_AUTH_TOKEN env var"
            )
